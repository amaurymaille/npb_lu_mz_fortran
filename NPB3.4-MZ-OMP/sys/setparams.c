...
#define GLOBAL_PREFIX_LEN 10

static char GLOBAL[GLOBAL_PREFIX_LEN];
...
void write_lu_info(FILE *fp, char class) 
{
  ...
  fprintf(fp, "%scharacter %sclass\n", FINDENT, GLOBAL);
  fprintf(fp, "%sparameter (%sclass='%c')\n", FINDENT, GLOBAL, class);
  fprintf(fp, "%sinteger %sx_zones, %sy_zones\n", FINDENT, GLOBAL,
          GLOBAL);
  fprintf(fp, "%sparameter (%sx_zones=%d, %sy_zones=%d)\n", FINDENT,
          GLOBAL, x_zones, GLOBAL, y_zones);
  fprintf(fp, "%sinteger %sgx_size, %sgy_size, %sgz_size\n", 
          FINDENT, GLOBAL, GLOBAL, GLOBAL);
  fprintf(fp, "%sparameter (%sgx_size=%d, %sgy_size=%d, \n     >%sgz_size=%d)\n", 
	       FINDENT, GLOBAL, gx_size, GLOBAL, gy_size,
           GLOBAL, gz_size);
  fprintf(fp, "%sinteger %sproblem_size, %skind2\n", FINDENT, GLOBAL, 
          GLOBAL);
  fprintf(fp, "%sparameter (%sproblem_size = %d, %skind2 = %d)\n", FINDENT, 
          GLOBAL, max(max_lsize,gz_size), GLOBAL, kind2);

  fprintf(fp, "\nc number of iterations and how often to print the norm\n");
  fprintf(fp, "%sinteger %sitmax_default, %sinorm_default\n", FINDENT,
          GLOBAL, GLOBAL);
  fprintf(fp, "%sparameter (%sitmax_default=%d, %sinorm_default=%d)\n", 
	  FINDENT, GLOBAL, itmax, GLOBAL, inorm);
  fprintf(fp, "%sdouble precision %sdt_default, %sratio\n", FINDENT,
          GLOBAL, GLOBAL);
  fprintf(fp, "%sparameter (%sdt_default = %s, %sratio = %s)\n", FINDENT, 
                GLOBAL, dt_default, GLOBAL, ratio);
}
...
void put_string(FILE *fp, char *name, char *val)
{
  ...
  fprintf(fp, "%scharacter %s%s*%d\n", FINDENT, GLOBAL, name, len);
  fprintf(fp, "%sparameter (%s%s=\'%s\')\n", FINDENT, GLOBAL, name, val);
}
...
void write_convertdouble_info(int type, FILE *fp)
{
  ...
  case BT:
  case LU:
    fprintf(fp, "%slogical %sconvertdouble\n", FINDENT, GLOBAL);
#ifdef CONVERTDOUBLE
    fprintf(fp, "%sparameter (%sconvertdouble = .true.)\n", FINDENT,
            GLOBAL);
#else
    fprintf(fp, "%sparameter (%sconvertdouble = .false.)\n", FINDENT,
            GLOBAL);
#endif
    break;
  }
