...
c$    call omp_set_dynamic(.false.)
      nthreads = global_num_othreads
c      write (*, 100) nthreads
      if (global_nested.eq.2) nthreads = global_num_threads
...
c ... info for current thread
      global_myid = 0
c$    global_myid = omp_get_thread_num()
      global_root = 0
c
c ... reorganize list of zones for this outer thread
      zone_count = 0
      do iz = 1, num_zones
         if (global_zone_proc_id(iz) .eq. global_myid) then
            zone_count = zone_count + 1
            proc_zone_id(zone_count) = iz
         endif
      end do
      proc_num_zones = zone_count
      if (zone_count .ne. global_proc_zone_count(global_myid+1)) then
         write(*,*) 'Warning: ',global_myid, ': mis-matched zone counts
     >-', 
     &           zone_count, global_proc_zone_count(global_myid+1)
      endif
c
      return
      end
...
c ... local variables
      ...
      integer entry_counts(global_max_zones)
...
c
      ...
      if (ios .gt. 0 .and. mp .gt. 0) then
         read(envstr,*,iostat=ios) global_num_othreads
         if (ios.ne.0 .or. global_num_othreads.lt.1) global_num_othreads
     >= 1
         if (mp .ne. global_num_othreads) then
            write(*, 10) global_num_othreads, mp
   10       format(' Warning: Requested ',i4,' outer-level threads,',
     &             ' but the active value is ',i4)
            global_num_othreads = mp
         endif
         read(envstr,*,iostat=ios) ip, global_num_threads
         if (ios.ne.0 .or. global_num_threads.lt.1) global_num_threads =
     >1
      else
         global_num_othreads = 1
         global_num_threads = 1
      endif
c
      if (global_num_othreads .gt. global_max_zones) then
         write(*, 15) global_num_othreads, global_max_zones
   15    format(' Error: global_num_othreads ',i5,
...
c ... check nested-par support
      ...
      if (ios .gt. 0 .and. mp .gt. 0) then
         read(envstr,*,iostat=ios) global_nested
         if (ios.ne.0 .or. global_nested.lt.0) global_nested = 0
      else
         global_nested = 0
      endif
      if (global_nested.eq.2) global_num_othreads = 1
      if (global_nested.eq.1) global_num_threads = 1
c
      if (global_nested.eq.1 .or. global_nested.eq.2) then
c$       call omp_set_nested(.false.)
      else 
c$       call omp_set_nested(.true.)
         nflag = .true.
c$       nflag = omp_get_nested()
         if ((.not.nflag) .and. global_num_threads.gt.1) then
            if (global_nested.eq.3) then
...
c
      ...
      if (ios .gt. 0) then
         if (envstr.eq.'on' .or. envstr.eq.'ON') then
            ...
            read(envstr,*,iostat=ios) global_mz_bload
            if (ios.ne.0) global_mz_bload = 0
         endif
      else
         global_mz_bload = 1
      endif
c
      ...
      global_npb_verbose = 0
      if (ios .gt. 0) then
         read(envstr,*,iostat=ios) global_npb_verbose
         if (ios.ne.0) global_npb_verbose = 0
      endif
c
      do ip = 1, global_num_othreads
         global_proc_num_threads(ip) = global_num_threads
         global_proc_group(ip) = 0
      end do
c
      ...
      if (ios.eq.0) then
         ...
         if (global_mz_bload .ge. 1) then
            global_mz_bload = -global_mz_bload
         endif
   ...
   25       read(2,'(a)',end=40,err=40) line
            ...
            if (global_mz_bload .lt. 0 .and. group .gt. 0) then
               global_mz_bload = -global_mz_bload
            endif
            ...
            if (ip1.lt.0) ip1 = 0
            if (ip2.ge.global_num_othreads) ip2 = global_num_othreads -
     >1

            do ip = ip1+1, ip2+1
               global_proc_num_threads(ip) = curr_threads
               global_proc_group(ip) = group
   ...
   40    close(2)
     ...
     &                    ip-1, ', only the last one used'
      ...
      if (ip1 .gt. 0 .or. global_npb_verbose .gt. 0) then
         ip1 = 0
         do ip = 1, global_num_othreads
            if (ip .eq. 1 .or.
     &          global_proc_num_threads(ip) .ne. curr_threads .or.
     &          global_proc_group(ip) .ne. group) then
     ...
     &            write(*,30) ip2, curr_threads, group
               ...
               curr_threads = global_proc_num_threads(ip)
               group = global_proc_group(ip)

               ip1 = ip - 1
               write(*,30) ip1, curr_threads, group

            else if (ip .eq. global_num_othreads) then
               ip2 = ip-1
               if (ip2 .gt. ip1+1) write(*,*) '    ...'
               write(*,30) ip2, curr_threads, group
            endif
         end do
   30    format('  othread',i6,'  global_num_threads =',i5,
     >          '  flag =',i5)
      endif
c
      tot_threads = 0
      if (mp .gt. 0) then
         do ip = 1, global_num_othreads
            tot_threads = tot_threads + global_proc_num_threads(ip)
         end do
c
         write(*, 1003) global_num_othreads
         write(*, 1004) tot_threads,
     >                  dble(tot_threads)/global_num_othreads
      endif
...
c
      jzone  = (zone - 1)/global_x_zones + 1
      izone  = mod(zone - 1, global_x_zones) + 1
      izone_west  = global_iz_west(zone)
      izone_east  = global_iz_east(zone)
      jzone_south = global_iz_south(zone)
      jzone_north = global_iz_north(zone)
c
      comm_index = 0
      if (global_zone_proc_id(izone_west) .eq. iproc)
     $   comm_index = comm_index + global_y_size(jzone)
      if (global_zone_proc_id(izone_east) .eq. iproc)
     $   comm_index = comm_index + global_y_size(jzone)
      if (global_zone_proc_id(jzone_south) .eq. iproc)
     $   comm_index = comm_index + global_x_size(izone)
      if (global_zone_proc_id(jzone_north) .eq. iproc)
     $   comm_index = comm_index + global_x_size(izone)
c
      return
      end
...
c ... use a simple bin-packing scheme to balance the load among othreads
      do ip = 1, global_num_othreads
         global_proc_zone_count(ip) = 0
         global_proc_zone_size(ip) = 0.d0
      end do
      do iz = 1, num_zones
         global_zone_proc_id(iz) = -1
      end do
...
c  ...   the current most empty thread
         np = 1
         cur_size = global_proc_zone_size(1)
         do ip = 2, global_num_othreads
            if (cur_size.gt.global_proc_zone_size(ip)) then
               np = ip
               cur_size = global_proc_zone_size(ip)
            endif
         end do
         ip = np - 1
c
...
c        the current group and does not worsen the computation balance
     ...
     &                      zone_size(zone)) / zone_size(z_order(iz))
               ...
               if (global_zone_proc_id(zone) .lt. 0) then
...
c  ...   assign the zone to the current thread group
  120    global_zone_proc_id(mz) = ip
         global_proc_zone_size(np) = global_proc_zone_size(np) +
     >zone_size(mz)
         global_proc_zone_count(np) = global_proc_zone_count(np) + 1
c
c  ...   skip the previously assigned zones
         do while (iz.le.num_zones)
            if (global_zone_proc_id(z_order(iz)).lt.0) goto 130
            iz = iz + 1
         end do
  130    continue
      end do
...
c ... move threads around if needed
      mz = 1
      if (tot_threads.le.global_num_othreads .or. global_mz_bload.lt.1)
     >mz = 0
c
      if (mz .ne. 0) then
c
         do ipg = 1, global_num_othreads
...
c ...    balance load within a thread group
  200    do while (ipg .le. global_num_othreads)
            if (proc_group_flag(ipg) .eq. 0) goto 210
            ipg = ipg + 1
         end do
  210    if (ipg .gt. global_num_othreads) goto 300
c
         group = global_proc_group(ipg)
         tot_group_size = 0.d0
         tot_group_threads = 0
         do ip = ipg, global_num_othreads
            if (global_proc_group(ip) .eq. group) then
               proc_group_flag(ip) = 1
               tot_group_size = tot_group_size +
     >global_proc_zone_size(ip)
               tot_group_threads = tot_group_threads + 
     &                             global_proc_num_threads(ip)
            endif
         end do
c
...
c  ...   distribute size evenly among threads
         icur_size = 0
         do ip = 1, global_num_othreads
            if (global_proc_group(ip) .ne. group) goto 220
            global_proc_num_threads(ip) = global_proc_zone_size(ip) /
     >ave_size
            if (global_proc_num_threads(ip) .lt. 1)
     &          global_proc_num_threads(ip) = 1
            if (global_max_threads .gt. 0 .and. 
     &          global_proc_num_threads(ip) .gt. global_max_threads) 
     &          global_proc_num_threads(ip) = global_max_threads
            icur_size = icur_size + global_proc_num_threads(ip)
  220       continue
         end do
...
c  ...   take care of any remainers
         ...
         do while (mz .ne. 0)
            ...
            imx = 0
            do ip = 1, global_num_othreads
               if (global_proc_group(ip) .ne. group) goto 230
               if (mz .gt. 0) then
                  cur_size = global_proc_zone_size(ip) /
     >global_proc_num_threads(ip)
                  if (cur_size.gt.max_size .and.
     >(global_max_threads.le.0
     &                .or.
     >global_proc_num_threads(ip).lt.global_max_threads)) then
                     max_size = cur_size
                     imx = ip
                  endif
               else if (global_proc_num_threads(ip) .gt. 1) then
                  cur_size = global_proc_zone_size(ip) / 
     &                       (global_proc_num_threads(ip)-1)
  ...
  230          continue
            ...
            global_proc_num_threads(imx) = global_proc_num_threads(imx)
     >+ inc
            mz = mz - inc
         end do
c
...
c
c ... print the mapping
  300 if (global_npb_verbose .gt. 0) then
         write(*,20)
         do ip = 1, global_num_othreads
            write(*,25) ip-1,global_proc_zone_count(ip),
     &           
     >global_proc_zone_size(ip),global_proc_num_threads(ip),
     &            global_proc_zone_size(ip)/global_proc_num_threads(ip)
            do iz = 1, num_zones
               if (global_zone_proc_id(iz) .eq. ip-1) then
...
c
      imx = 1
      max_size = global_proc_zone_size(1)/global_proc_num_threads(1)
      imn = imx
      ave_size = max_size
      do ip = 2, global_num_othreads
         cur_size =
     >global_proc_zone_size(ip)/global_proc_num_threads(ip)
      ...
      if (global_npb_verbose .gt. 0) then
         write(*,*)
         write(*,35) 'Max', imx-1, global_proc_zone_count(imx),
     &              
     >global_proc_zone_size(imx),global_proc_num_threads(imx)
         write(*,35) 'Min', imn-1, global_proc_zone_count(imn),
     &              
     >global_proc_zone_size(imn),global_proc_num_threads(imn)
      endif
