...
c---------------------------------------------------------------------
     ...
     $                 phi1(global_problem_size,global_problem_size),
     $                 phi2(global_problem_size,global_problem_size)
...
!$OMP DO SCHEDULE(STATIC)
      do j = 2,ny-2
         do i = 2,nx-1
            ...
            phi1(i,j) = global_c2*(u(5,i,j,k) -
     >             0.5d0 * (u(2,i,j,k)**2 + u(3,i,j,k)**2 +
     >                      u(4,i,j,k)**2) / u(1,i,j,k) )

            k = nz-1

            phi2(i,j) = global_c2*(u(5,i,j,k) -
     >             0.5d0 * (u(2,i,j,k)**2 + u(3,i,j,k)**2 +
     >                      u(4,i,j,k)**2) / u(1,i,j,k) )
         end do
      end do
...
!$OMP ATOMIC
      frc = frc + global_dxi * global_deta * frc1
...
!$OMP DO SCHEDULE(STATIC)
      do k = 3, nz-1
         do i = 2, nx-1
            phi2(i,k) = global_c2*(u(5,i,ny-2,k) -
...
!$OMP ATOMIC
      frc = frc + global_dxi * global_dzeta * frc2
...
!$OMP DO SCHEDULE(STATIC)
      do k = 3, nz-1
         do j = 2, ny-2
            phi2(j,k) = global_c2*(u(5,nx-1,j,k) -
...
!$OMP ATOMIC
      frc = frc + global_deta * global_dzeta * frc3
!$OMP END PARALLEL
