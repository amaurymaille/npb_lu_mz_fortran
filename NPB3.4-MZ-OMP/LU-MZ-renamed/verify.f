...
c---------------------------------------------------------------------
        ...
        write(*, 1990) global_class
 1990   format(/, ' Verification being performed for global_class ', a)
        write (*,2000) epsilon
 2000   format(' Accuracy setting for epsilon = ', E20.13)
        if (dabs(global_dt-dtref) .gt. epsilon) then  
           verified = .false.
           write (*,1000) dtref
 1000      format(' DT does not match the reference value of ', 
     >              E15.8)
        else if (global_itmax .ne. itmaxref) then
           verified = .false.
           write (*,1002) itmaxref
